...
Ticket #104718 (oksbot-scl-hamamatsu105, 2026-04-22) revealed a failure mode that the existing
learning material didn't cover: a robot parked at a **tape intersection** caused all 4 sensorbars
...
The sensorbar estimator fits a bell curve (actually a super-Gaussian trough) to the reflectance profile:
...
This is the opposite of what you'd intuitively expect. The pull-down circuit in the sensorbar
...
| 05:21:41 | **`EstimatorReflectanceBell: Curve fitting disabled sensors for sensorbar: 4 (max: 2)`** |
| 05:21:46 | Sensorbar calibration: 4/4 bars HEALTHY (not the issue) |
...
Use this checklist when you see `MCLBC_Navigation code 1` or `EstimatorReflectanceBell: Curve fitting disabled sensors for sensorbar: N (max: 2)`:
...
□ Does /rosout show "Curve fitting disabled sensors" for ALL 4 bars simultaneously?
  ...
  → If only 1-2 bars: more likely single sensorbar hardware degradation

□ Was there a recent restart/reboot before the failure?
  → Cold-start required to trigger this pattern (no cached solution)

□ Check the actual sensor values from /robot<N>/sensorbars at failure time
...
□ Run sensorbar health check (is_reliable dropout, stuck sensors)
