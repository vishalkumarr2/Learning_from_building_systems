...
**A1.** `ERROR_INVALID_STATE` appears in every OKS incident ticket involving slip. A new engineer says "so the bug is in 's isFinite() check — we should remove it so the robot can keep moving after a slip." Why is this wrong, and what would actually happen if isFinite() were removed?
...
**The correct mental model:** After a slip, the robot genuinely does not know where it is. Allowing it to navigate with `cov=INF` is not "keep moving" — it is "move blindly with no position knowledge." The safe response is to stop and wait for re-localization via sensorbar.

**The fix is not to remove the check.** The fix is to address the root cause of the slip (floor, motor driver, etc.) and ensure there is a sensorbar line nearby for the robot to cross and recover.

</details>

---

**A2.** An incident bag shows `status=MISSED_LINE_UPDATES (8)` for 8 seconds, then `status=DELOCALIZED (1)` for 3 seconds, then the robot stops. During the entire 11 seconds, the sensorbar topic was publishing at 100 Hz with `is_reliable=True`. Explain the complete sequence of events in the estimator and why two different status codes appear.

<details><summary>Answer</summary>

**MISSED_LINE_UPDATES phase (0–8s):**

The sensorbar was physically detecting lines and publishing measurements, but the EKF was rejecting every measurement via the innovation gate (chi-squared / Mahalanobis distance check). This can happen because:
- The estimator's position estimate diverged slightly and each sensorbar measurement looked like an outlier
...
During this phase, the estimator kept running but received no useful sensorbar updates. The prediction step (`predict()`) continued adding motion noise each odom cycle, but no update step (`update()`) ran to reduce the covariance. So covariance grew monotonically.

**Why status=8 and not DELOCALIZED immediately:** The covariance had not yet crossed the DELOCALIZED threshold. Status=8 is reported as soon as sensorbar updates stop being accepted, which can happen with covariance still below the threshold.

**DELOCALIZED phase (8–11s):**

After 8 seconds without sensorbar corrections, the covariance finally crossed the `delocalized_threshold` (typically 0.03–0.1 depending on config). Status transitions to 1 (DELOCALIZED). The estimator continues running and reporting position but with lower confidence.
...
(b) Robot dead-reckons for 20s without sensorbar updates, covariance grows to 0.04 (DELOCALIZED threshold).

Include: what `cov_xx` looks like over time, what `status` looks like, what the velocity signal looks like, and what happens when the robot next crosses a sensorbar line.
...
After sensorbar crossing: `cov_xx` **drops sharply** from INF back to the measurement quality (≈ sensorbar noise, e.g. 0.0001). Status → 0 (OK). This is an abrupt, large decrease.
...
After sensorbar crossing: `cov_xx` **drops from ~0.04 to ~0.001** in one update. Status → 0 (OK). A moderate, expected decrease.
...
**A5.** Two engineers are debugging separate incidents. Engineer A says: "Status=8 means the SPI bus is broken." Engineer B says: "Status=8 means the sensorbar is physically off the lines." Who is right, and what is the precise condition that sets status=8?
...
Status is set to 8 when the estimator detects that a **statistically expected sensorbar update has not been incorporated** for longer than a threshold period. Specifically: the sensorbar topic is alive (messages arriving) but the EKF update step has not accepted any measurement.
...
3. **Large estimator position error:** If the estimated position diverged badly, real sensorbar measurements look like outliers and are rejected by the Mahalanobis gate
...
- Check if sensorbar detections contain line position data or are empty → Engineer B's cause (empty = NO_LINE, not MISSED_UPDATES)
...
[12:34:15.263] /rosout [ERROR]: ERROR_INVALID_STATE
[12:34:15.263] /cmd_vel: linear.x=0.0  <══ stop command
[12:34:15.400] /sensorbar: rate=0Hz (no messages)
```
...
- `ERROR_INVALID_STATE` follows immediately (isFinite(cov_xx=1e6) fails)
...
- The sensorbar going silent *after* the stop is a consequence, not a cause
...
- After fixing root cause: verify a sensorbar line is accessible for the robot to cross and recover
...
[09:15:25.000] /estimated_state: status=1, cov_xx=0.0381, cov_yy=0.0373
                ↑ status just changed!
[09:15:25.000] /sensorbar: rate=98Hz, is_reliable=True, detections=[]
[09:15:25.000] /odom: rate=49Hz (normal)
[09:15:35.000] /estimated_state: status=1, cov_xx=0.0521, cov_yy=0.0508
[09:15:37.800] /sensorbar: is_reliable=True, detections=[line_at_offset=-0.012]
...
- Covariance drops sharply back to 0.0014 when sensorbar crossing occurs at 09:15:37.8

**(2) Root cause:**
- **Primary:** Long path segment with no sensorbar line crossings — 25+ seconds of dead-reckoning
- The sensorbar was *alive and healthy* (`rate=98Hz, is_reliable=True`) but reporting empty detections (`detections=[]`) — this is NO_LINE during the segment, meaning the robot was on a section of floor with no lines, or the lines were not within the sensorbar detection range
...
- Fix: add sensorbar lines to the floor segment, or reroute through a section with better line coverage
...
- Note: **`ERROR_INVALID_STATE` is NOT logged** — this uses the collision stop path, not the isFinite() path
...
[11:05:10.000] /sensorbar: rate=102Hz, is_reliable=True, detections=[line_at=-0.001]
[11:05:15.000] /estimated_state: status=0, cov_xx=0.0072
[11:05:15.000] /sensorbar: rate=99Hz, is_reliable=True, detections=[line_at=-0.002]
[11:05:20.000] /estimated_state: status=8, cov_xx=0.0198
[11:05:20.000] /sensorbar: rate=101Hz, is_reliable=True, detections=[line_at=-0.004]
                ↑ status=8 but sensorbar looks healthy!
[11:05:25.000] /estimated_state: status=8, cov_xx=0.0312
[11:05:25.000] /sensorbar: rate=100Hz, is_reliable=True, detections=[line_at=-0.004]
                                                                      ↑ same value as 5s ago?
[11:05:30.000] /rosout [WARN]: SPI transaction timeout on sensorbar_controller
[11:05:30.000] /sensorbar: rate=97Hz, is_reliable=True, detections=[line_at=-0.004]
...
**Evidence:**
- Sensorbar topic appears healthy: rate=100Hz, `is_reliable=True`
...
- SPI warning at T=11:05:30 confirms: `SPI transaction timeout on sensorbar_controller`
- Covariance grows slowly (not a slip step-jump) → eventually crosses DELOCALIZED threshold at T=11:05:35

**(2) Root cause:**
- **SPI bus conflict or sensorbar controller fault** causing the sensorbar to repeat the same measurement with the same timestamp
...
**(3) What to check next:**
- Check SPI error logs on the robot — is there a conflicting SPI device on the same bus as the sensorbar?
- Verify sensorbar measurement timestamps in the topic: `rostopic echo /sensorbar/detections | grep stamp` — are multiple consecutive messages reporting the same header.stamp?
...
You are given a bag from an incident at Hamamatsu. The following time series was extracted:

```
    Time     cov_xx    status    sensorbar_rate    notes
    ──────   ──────    ──────    ──────────────    ─────────────────────────
    T=10.3s  0.0012    0 (OK)    100 Hz
    T=10.4s  0.0012    0 (OK)    0 Hz              ← SENSORBAR GAP (1 sample)
    T=10.5s  0.0013    0 (OK)    100 Hz            ← sensorbar resumes
    ...
    T=12.8s  0.0004    0 (OK)    100 Hz            ← sensorbar crossing, cov reset
```

Note: sensorbar was not publishing for exactly 0.1 seconds (one sample) at T=10.4s.

**Question C1:** What happened at T=10.4s? Give a precise mechanistic explanation of why `cov_xx` is still 0.0012 at T=10.5s even though a sensorbar gap occurred.

<details><summary>Answer</summary>

**At T=10.4s:** The sensorbar topic published no message for one sample period (0.1s gap). This could be caused by:
- A brief SPI timeout that missed one transmission cycle
- A packet drop on the internal ROS network
- A single-cycle CPU scheduling delay causing the sensorbar publisher to miss its deadline

**Why cov_xx stays at 0.0012 at T=10.5s:**

The covariance contribution from a missing sensorbar update is **very small** compared to the contribution from motion. During 0.1 seconds without a sensorbar update, the prediction step ran (with odom still arriving at 50Hz) and added motion noise. But the motion noise per prediction step is proportional to `delta_trans × k_trans_noise`. For a robot moving at ~0.3 m/s:
...
This is below the display precision in the data extract (rounds to 0.0012 or would show as 0.0013). The sensorbar also resumed at T=10.5s and provided a correction that pulled `cov_xx` back down, masking the 0.1s gap entirely. A single 0.1s sensorbar gap is **below the detection threshold** for this measurement — it is absorbed into normal covariance fluctuation.

**The key insight:** The covariance increase from a 0.1s sensorbar gap is negligible. The covariance increase visible from T=10.5s to T=12.6s (roughly +0.002 per 0.5s increment) represents **2.1 seconds of sustained dead-reckoning after T=10.5s** — meaning sensorbar updates resumed but the robot was NOT crossing any sensorbar lines during this period (just the topic publishing at 100Hz with no line crossings under the sensor, i.e., traversing blank floor).

</details>

---

**Question C2:** The sensorbar rate returned to 100Hz at T=10.5s. Yet status did NOT transition to `MISSED_LINE_UPDATES (8)`. Why not? What would need to be true for status=8 to appear here?

<details><summary>Answer</summary>

**Why status=8 did NOT appear:**

Status=8 (MISSED_LINE_UPDATES) requires that the estimator is **expecting** sensorbar updates and not receiving them. Specifically, it tracks whether any updates have been *accepted* by the EKF over a time window.

At T=10.5s, the sensorbar resumed publishing at 100Hz. However, the topic was publishing — it just had no **line detections** in the messages (the `detections=[]` list was empty). This is **NO_LINE behaviour, not MISSED_LINE_UPDATES**. The robot was traversing floor with no lines underneath the sensorbar, so the sensorbar correctly reported "I see nothing." An empty detection is not a "missed update" — it is an accurate report.

For status=8 to appear, the sensorbar would need to be **reporting line detections** (`detections=[line_at=xxx]`) but those detections would need to be **rejected by the EKF** (failing the innovation gate). In this scenario, the detections list appears to be empty during the T=10.5–12.6s window (inferred from the fact that covariance grows steadily without any corrections).
...
- The floor between T=10.5s and T=12.6s would need to have sensorbar lines painted on it
- The sensorbar would need to be detecting those lines (non-empty detections)
...
**Question C3:** If the sensorbar gap at T=10.4s had been **20 seconds** instead of 0.1 seconds (sensorbar offline from T=10.4s to T=30.4s), and the robot continued moving at the same speed, what would the failure sequence look like? Estimate:
- When would DELOCALIZED first appear?
- Would ERROR_INVALID_STATE fire? Under what condition?
- What is the earliest time at which the robot could recover?

<details><summary>Answer</summary>

**Estimating covariance growth during 20s sensorbar gap:**
...
```
    ...
    (much earlier than in the original scenario because there are no sensorbar
    corrections to slow the growth at all during the 20s gap)
```

**Would ERROR_INVALID_STATE fire?**
...
ERROR_INVALID_STATE fires on **non-finite** (INF or NaN) values. Pure dead-reckoning covariance growth stays finite (just large). So NAV_NOT_FINITE would NOT fire from dead-reckoning alone, assuming no slip event.
...
The robot can recover as soon as it crosses a sensorbar line AND the EKF accepts the measurement. With `cov_xx` now very large (but finite), the Kalman gain `K` would be very large, meaning the sensorbar measurement would completely dominate (almost ignore the prior estimate). The recovery would happen in **one sample** of the sensorbar.

```
    Earliest recovery = T=30.4s + (time to first sensorbar line crossing)
```

If a sensorbar line was at the same location as in the original scenario (T=12.8s corresponds to approximately 2.4s of travel distance at 0.3 m/s = ~0.72m past the gap start), the robot would cross that line at T=30.4s + 2.4s (if it keeps moving) = approximately T=32.8s.
...
This illustrates why even brief sensorbar gaps matter when they happen at the start of a long blank-floor segment — the covariance trajectory is the same, but the time to recovery increases dramatically.
